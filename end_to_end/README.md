...
- Clerk-backed production FastAPI backend for `/me`, `/score`, `/cases`,
  `/cases/{transaction_id}`, `/cases/{transaction_id}/decision`,
  `/cases/{transaction_id}/rescore`, `/cases/{transaction_id}/feedback`,
  `/cases/{transaction_id}/gemini-analysis`, `/metrics/summary`, and
  `/monitoring/summary`
- Supabase/Postgres-ready ORM persistence for cases, score runs, analyst
  reviews, feedback, users, audit logs, monitoring events, MLflow tracking
  metadata, and Gemini advisory payloads
